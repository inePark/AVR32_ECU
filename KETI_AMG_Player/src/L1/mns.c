...
#define RETRY_DONE(flag)     { TAKE_MNS(); mns.retry &= ~flag; GIVE_MNS(); }
...
#ifdef MNS_26
void MnsHandleVersionInfo(TMsgRx *msg_ptr)
{
    bool okay;
...
#ifdef MNS_27
void MnsHandleEHCIStateStatus(TMsgRx *msg_ptr)
{
    byte current_state;
...
/*! Retry handler for the MNS module ("on buffer freed" event).
  *
  * @author     SSemmler
  * @date       last update  4/28/2005
  * @version    2.0.3
  *
  * @return     ERR_NO or ERR_BUFOV (retry is scheduled)
  */
...
#ifdef MNS_53
void MnsHandleWatchdogModeStatus(TMsgRx *msg_ptr)
{
    word new_timeout;
    word new_wdsm;
...
byte MnsDisableSCError(void)
{
    ...
    while (count < 3)
    {
        msg_ptr[count] = MsgGetTxPtrExt(0);
        if(msg_ptr[count])
        {
