...
#endif
#ifdef MAX_MSG_TX_DATA
    /*!
      * \brief      Legacy function to request a Tx message with a
      *             fixed payload buffer.
      * \details    This function is only available if \c DECLARE_LEGACY_API
      *             is defined. \c MAX_MSG_TX_DATA specifies the size of payload.
      * \return     Possible return values:
      *             - Pointer to a free message buffer.
      *             - \c NULL if there is no free buffer.
      */
    #define MsgGetTxPtr() MsgGetTxPtrExt((word)MAX_MSG_TX_DATA)
#endif
...
#endif
#ifdef MAX_MSG_RX_DATA
    /*!
        * \brief      Legacy function to request an Rx message with a
        *             fixed payload buffer.
        * \details    This function is only available if \c DECLARE_LEGACY_API
        *             is defined. \c MAX_MSG_RX_DATA specifies the size of payload.
        * \return     Possible return values:
        *             - Pointer to a free message buffer.
        *             - \c NULL if there is no free buffer.
        */
    #define MsgGetRxInPtr() MsgGetRxInPtrExt(MAX_MSG_RX_DATA)
#endif
...
#ifdef NS_AMS_AH
    #define MsgGetTxPtrRes() MsgGetTxPtr()
#endif
