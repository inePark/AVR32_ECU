...
*/
           ...
           #if MAX_MSG_TX_DATA > PMS_ALLOC_MAX_SIZE
               #error "MAX_MSG_TX_DATA needs to be smaller than PMS_ALLOC_MAX_SIZE"
           #endif

           #if MAX_MSG_RX_DATA > PMS_ALLOC_MAX_SIZE
               #error "MAX_MSG_RX_DATA needs to be smaller than PMS_ALLOC_MAX_SIZE"
           #endif

           #if MAX_MSG_TX_DATA <= 11
               #error "MAX_MSG_TX_DATA needs to be bigger than 11"
           #endif

           #if MAX_MSG_RX_DATA <= 11
               #error "MAX_MSG_RX_DATA needs to be bigger than 11"
           #endif
        #endif
    #endif
#endif  /* #ifdef PMS_MIN */
...
--------------------------------------------------------------------------------
*/
#define PMS_EXTERNAL_EVENT(e) if( pmsInitData_ptr->cbExtEvents_fptr ) pmsInitData_ptr->cbExtEvents_fptr(e)
...
typedef struct PmsStruct
{
    ...
    unsigned int initSync        : 1;
    unsigned int ifacesStarted   : 1;
#ifndef PACKET_COMMUNICATION_ONLY
    #ifndef PMS_TX_NOSEG
    unsigned int needShadow      : 1;
    #endif

    #ifdef MSG_RX_AUTOMATIC_ERR_MSG
    unsigned int needSegErrMsg   : 1;
    #endif

    #ifdef PMS_RX_SEND_ACK
    unsigned int needRetriggerAck: 1;
    #endif
    unsigned int needPrealloc    : 1;
    unsigned int rxPreAllocHasTimedOut : 1;
    unsigned int rxPreAllocFailedTimerStarted : 1;
